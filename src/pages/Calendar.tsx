            ...
            {days.map((day, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-0.5 flex items-center justify-center gap-1 pointer-events-none select-none">
                  ...
                  return dayMood ? (
                    <div className="w-full mt-3 bg-primary/5 border-[3px] border-border border-dashed rounded-2xl p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-card border-[3px] border-border flex items-center justify-center text-xl shadow-sm overflow-hidden flex-shrink-0">
                          {imgErrors[dayMood] ? (
                        ...
                        <div className="text-left">
                          <p className="text-[10px] font-black uppercase tracking-wider text-primary">
                            {lang === 'ru' ? 'Настроение дня' : 'Daily Mood'}
                          </p>
                          <p className="text-sm font-black text-foreground leading-snug">
                            {t(moodTr[`level${dayMood}` as keyof typeof moodTr], lang)}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="w-full mt-3 bg-primary/5 border-[3px] border-border border-dashed rounded-2xl p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-2.5 flex-1">
                        <span className="text-xl select-none">🤔</span>
                        <p className="text-xs text-muted-foreground font-bold text-left">
                          {lang === 'ru' ? 'Настроение на этот день не записано.' : 'No mood logged for this day.'}
                        </p>
                      </div>
                      <button
                        onClick={() => setIsMoodometerOpen(true)}
